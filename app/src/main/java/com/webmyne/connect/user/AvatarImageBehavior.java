...
public class AvatarImageBehavior extends CoordinatorLayout.Behavior<CircleImageView> {
    ...
    private void bindDimensions() {
        mAvatarMaxSize = mContext.getResources().getDimension(R.dimen.profile_image);
    }
    ...
    private void maybeInitProperties(CircleImageView child, View dependency) {
        ...
        if (finalHeight == 0)
            finalHeight = mContext.getResources().getDimensionPixelOffset(R.dimen.final_profile_image);

        if (mStartXPosition == 0)
            mStartXPosition = (int) (child.getX() + (child.getWidth() / 2));

        if (mFinalXPosition == 0)
            mFinalXPosition = mContext.getResources().getDimensionPixelOffset(R.dimen.space) + (finalHeight / 2);
